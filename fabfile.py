...
def test(package=''):
    ...
    if COVERAGE_ENABLED:
        ...
        result = _execute("DJANGO_SETTINGS_MODULE=project.test_settings coverage run --rcfile=.coveragerc manage.py test {}".format(PROJECT_PACKGE))
        _execute("coverage html --rcfile=.coveragerc")
        _execute("coverage xml --rcfile=.coveragerc")
    else:
        result = _execute('DJANGO_SETTINGS_MODULE=project.test_settings python manage.py test {}'.format(PROJECT_PACKGE))
